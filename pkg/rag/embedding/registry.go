...
func (r *DefaultRegistry) registerBuiltinGenerators() {
	...
	if err := r.Register("all-minilm-l6-v2", func(config VectorGeneratorConfig) (VectorGenerator, error) {
		return NewAllMiniLML6V2Generator(config)
	}); err != nil {
		panic(fmt.Sprintf("Failed to register all-minilm-l6-v2: %v", err))
	}

	// Register GTE-small-zh
	if err := r.Register("gte-small-zh", func(config VectorGeneratorConfig) (VectorGenerator, error) {
		return NewGTEsmallZhGenerator(config)
	}); err != nil {
		panic(fmt.Sprintf("Failed to register gte-small-zh: %v", err))
	}

	// Register stsb-bert-tiny
	if err := r.Register("stsb-bert-tiny", func(config VectorGeneratorConfig) (VectorGenerator, error) {
		return NewSTSBBertTinyGenerator(config)
	}); err != nil {
		panic(fmt.Sprintf("Failed to register stsb-bert-tiny: %v", err))
	}

	// Register adapter for existing LocalEmbedder
	if err := r.Register("legacy-local", func(config VectorGeneratorConfig) (VectorGenerator, error) {
		legacyConfig := &Config{
	...
	if err := r.Register("hash-fallback", func(config VectorGeneratorConfig) (VectorGenerator, error) {
		...
		panic(fmt.Sprintf("Failed to register hash-fallback: %v", err))
	}

	// Register Cybertron-based models (stub implementation for now)
	if err := r.Register("all-minilm-l6-v2-cybertron", func(config VectorGeneratorConfig) (VectorGenerator, error) {
		return NewCybertronMiniLML6V2(config)
	}); err != nil {
		panic(fmt.Sprintf("Failed to register all-minilm-l6-v2-cybertron: %v", err))
	}

	if err := r.Register("gte-small-zh-cybertron", func(config VectorGeneratorConfig) (VectorGenerator, error) {
		return NewCybertronGTEsmallZh(config)
	}); err != nil {
		panic(fmt.Sprintf("Failed to register gte-small-zh-cybertron: %v", err))
	}

	if err := r.Register("stsb-bert-tiny-cybertron", func(config VectorGeneratorConfig) (VectorGenerator, error) {
		return NewCybertronSTSBbertTiny(config)
	}); err != nil {
		panic(fmt.Sprintf("Failed to register stsb-bert-tiny-cybertron: %v", err))
	}
}
