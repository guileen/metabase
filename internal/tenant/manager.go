...
import (
	...
	"github.com/metabase/metabase/internal/auth"
	"github.com/metabase/metabase/internal/common"
)
...
func (tm *TenantManager) CreateTenant(ctx context.Context, tenant *Tenant) error {
	...
	if exists, err := tm.tenantExists(ctx, tenant.Name, tenant.Domain); err != nil {
		...
		return common.NewInvalidInputError("Tenant with this name or domain already exists")
	}
...
func (tm *TenantManager) CreateProject(ctx context.Context, project *Project) error {
	...
	if exists, err := tm.projectExists(ctx, project.TenantID, project.Name); err != nil {
		...
		return common.NewInvalidInputError("Project with this name already exists")
	}
...
func (tm *TenantManager) GetProject(ctx context.Context, projectID string) (*Project, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, common.NewNotFoundError("project")
		}
...
func (tm *TenantManager) AddProjectMember(ctx context.Context, projectID, userID string, role MemberRole) error {
	...
	for _, member := range project.Members {
		if member.UserID == userID {
			return common.NewInvalidInputError("User is already a project member")
		}
	}
...
func (tm *TenantManager) RemoveProjectMember(ctx context.Context, projectID, userID string) error {
	...
	if !found {
		return common.NewNotFoundError("Project member")
	}

	// Don't allow removing the owner
	if project.OwnerID == userID {
		return common.NewInvalidInputError("Cannot remove project owner")
	}
...
func (tm *TenantManager) validateTenant(tenant *Tenant) error {
	if tenant.Name == "" {
		return common.NewInvalidInputError("Tenant name is required")
	}
	if len(tenant.Name) > 100 {
		return common.NewInvalidInputError("Tenant name too long (max 100 characters)")
	}
	if tenant.Domain != "" && len(tenant.Domain) > 255 {
		return common.NewInvalidInputError("Domain too long (max 255 characters)")
	}
	return nil
}
...
func (tm *TenantManager) validateProject(project *Project) error {
	if project.Name == "" {
		return common.NewInvalidInputError("Project name is required")
	}
	if len(project.Name) > 100 {
		return common.NewInvalidInputError("Project name too long (max 100 characters)")
	}
	if project.TenantID == "" {
		return common.NewInvalidInputError("Tenant ID is required")
	}
	if project.OwnerID == "" {
		return common.NewInvalidInputError("Owner ID is required")
	}
	return nil
}
